<?php
/**
 * Производит поиск подстроки $subStrToDel в строке $string с конца строки. Удаляет лишние и возвращает строку.
 ...
 * @return string
 */
function searchLastSymbol($string, $subStrToDel, $subStrToCheck)
{
    if (isset($string) && $string != '' && isset($subStrToDel) && $subStrToDel != '' && isset($subStrToCheck) && $subStrToCheck != '') {
        ...
        for ($i = substr_count($string, $subStrToDel); $i; $i--) {
            ...
            $posSubStrToDel = mb_strrpos($string, $subStrToDel);
            if ($posSubStrToDel !== FALSE) {
                ...
                $posSubStrToCheck = mb_strrpos($string, $subStrToCheck);
                if ($posSubStrToCheck !== FALSE) {
                    ...
                    $posTwoSubStrToDel = mb_strrpos($string, $subStrToDel);
                    // Обновляем позицию метки удаляемого символа
                    $posSubStrToDel = mb_strrpos($string, $arrSpecCode['specSymbolDel']);
                    ...
                    if ($posTwoSubStrToDel !== FALSE) {
                        ...
                        if (($posSubStrToDel + $posSubStrToCheck) > ($posTwoSubStrToDel + $posSubStrToCheck) && $posSubStrToDel > $posSubStrToCheck && $posTwoSubStrToDel > $posSubStrToCheck) {
                            ...
                            $posSubStrToCheck = mb_strrpos($string, $arrSpecCode['specSymbolTempC']);
